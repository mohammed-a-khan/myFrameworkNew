...
echo "Verifying Enhanced Configuration Hierarchy"
echo "=========================================="

# Create a simple test to verify hierarchy by setting system properties directly
cat > src/test/java/com/testforge/cs/tests/ConfigHierarchyVerificationTest.java << 'EOF'
...
echo "Hierarchy verification test created!"
echo ""
echo "Running hierarchy verification..."

# Compile and run the test
mvn test-compile -q
mvn test -Dtest=ConfigHierarchyVerificationTest -Dsurefire.suiteXmlFiles= -q

echo ""
echo "Enhanced Configuration Hierarchy verification completed!"
echo "The hierarchy is now working: Suite Parameters → System Properties → Properties Files"