...
public class CSConfigManager {
    ...
    /**
     * Load all object repository files from configured locations
     * Supports:
     * 1. Single file path: loads that specific file
     * 2. Directory path: loads all .properties files from that directory and subdirectories
     * 3. Multiple paths: comma-separated list of files/directories
     * 
     * Configuration property: cs.object.repository.paths
     * Default value: object-repositories
     * 
     * Examples:
     * - Single file: cs.object.repository.paths=object-repositories/main.properties
     * - Single folder: cs.object.repository.paths=object-repositories
     * - Multiple: cs.object.repository.paths=object-repositories,config/locators.properties,external/repos
     */
    private void loadObjectRepositories() {
        // Get configured object repository paths (default to "object-repositories" if not specified)
        String repositoryPaths = mergedProperties.getProperty("cs.object.repository.paths", "object-repositories");
        
        // If empty or blank, skip loading
        if (repositoryPaths == null || repositoryPaths.trim().isEmpty()) {
            logger.info("Object repository loading disabled (cs.object.repository.paths is empty)");
            return;
        }
        
        // Process each path (comma-separated)
        ...
        for (String path : paths) {
            ...
            if (!pathFile.exists()) {
                logger.warn("Object repository path not found: {}", path);
                continue;
            }
            ...
            if (pathFile.isFile()) {
                ...
                if (path.endsWith(".properties")) {
                    ...
                    logger.info("Loaded object repository file: {}", path);
                } else {
                    logger.warn("Skipping non-properties file: {}", path);
                }
            } else if (pathFile.isDirectory()) {
                // It's a directory - load all properties files recursively
                loadRepositoryFilesFromFolderRecursively(pathFile);
                logger.info("Loaded all repositories from directory: {}", path);
            }
        }
        
        // Log summary
        logger.info("Object repository loading complete. Total properties loaded: {}", mergedProperties.size());
    }
    
    /**
     * Recursively load all .properties files from a folder and its subfolders
     */
    private void loadRepositoryFilesFromFolderRecursively(File folder) {
        ...
        for (File file : files) {
            if (file.isDirectory()) {
                ...
                loadRepositoryFilesFromFolderRecursively(file);
            } else if (file.isFile() && file.getName().endsWith(".properties")) {
                // Load properties file
                loadPropertiesFromFile(file.getPath());
                logger.debug("Loaded repository from: {}", file.getPath());
            }
        }
    }
