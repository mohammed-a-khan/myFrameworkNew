...
REM Check if PAT token is set
if "%ADO_PAT_TOKEN%"=="" (
    echo ERROR: ADO_PAT_TOKEN environment variable is not set!
    echo Please set it using: set ADO_PAT_TOKEN=your_pat_token_here
    echo.
    pause
    exit /b 1
)
...
REM Run the ADO BDD test suite (CSBDDRunner will handle ADO automatically when enabled)
